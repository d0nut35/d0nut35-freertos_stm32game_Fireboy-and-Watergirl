...
static QueueHandle_t g_xQueueMPU6050; /* MPU6050队列句柄 */
static EventGroupHandle_t g_xEventMPU6050; /*利用事件组节省资源*/
...
/**********************************************************************
 * 函数名称： mpu6050_task
 * 功能描述： MPU6050任务,它循环读取MPU6050并把数值写入队列,当中断出发时才写入事件组进行该任务读mpu6050的值
 * 输入参数： params - 未使用
 * 输出参数： 无
 *            无
 * 返 回 值： 0 - 成功, 其他值 - 失败
