 ...
 * PB1  - Key
 */
#define ROTARY_ENCODER_S1_GPIO_GROUP GPIOA
#define ROTARY_ENCODER_S1_GPIO_PIN   GPIO_PIN_4

#define ROTARY_ENCODER_S2_GPIO_GROUP GPIOA
#define ROTARY_ENCODER_S2_GPIO_PIN   GPIO_PIN_5

#define ROTARY_ENCODER_KEY_GPIO_GROUP GPIOA
#define ROTARY_ENCODER_KEY_GPIO_PIN   GPIO_PIN_6

static int32_t g_count = 0; 
static int32_t g_speed = 0; /* 速度(正数表示顺时针旋转,负数表示逆时针旋转,单位:每秒转动次数) */

static QueueHandle_t g_xQueueRotary;   /* 旋转编码器队列句柄 */
static uint8_t g_ucQueueRotaryBuf[ROTARY_QUEUE_LEN*sizeof(struct rotary_data)];
static StaticQueue_t g_xQueueRotaryStaticStruct;//必须执行一个 StaticQueue_t 结构体，用来保存队列的数据结构
...
void RotaryEncoder_IRQ_Callback(void)
{
    ...
    static volatile uint64_t pre_time = 0;
	struct rotary_data rdata;
	
	BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        
	/* 1. 记录中断发生的时刻 */	
	time = system_get_us();//system_get_ns();
	...
	//g_speed = (uint64_t)1000000/(time - pre_time);
	g_speed = (uint64_t)1000/(time - pre_time);
...
//		LCD_ShowNum(30,70,count,10,20);
//		LCD_ShowNum(30,110,speed,10,20);
//		LCD_ShowNum(30,150,key,10,20);
