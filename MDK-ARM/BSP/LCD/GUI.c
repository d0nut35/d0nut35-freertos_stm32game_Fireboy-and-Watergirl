...
void LCD_Show2Num(uint16_t x,uint16_t y,int16_t num,uint8_t len,uint8_t size)
{
	uint8_t t,temp;
	uint8_t enshow=0;
	if(num<0)
 ...
 * @retvalue   :None
******************************************************************************/ 
void Gui_Drawbmp16(uint16_t x,uint16_t y,const unsigned char *p,uint16_t size_x,uint16_t size_y) //显示40*40 QQ图片
{
  	int i; 
